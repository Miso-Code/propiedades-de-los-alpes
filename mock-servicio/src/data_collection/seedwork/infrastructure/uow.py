...
from src.data_collection.seedwork.domain.entities import RootAggregate
...
def register_unit_of_work(serialized_obj):
    from src.data_collection.config.uow import SQLAlchemyUnitOfWork
    from flask import session
...
def flask_uow():
    ...
    if session.get('uow'):
        ...
        uow_serialized = pickle.dumps(SQLAlchemyUnitOfWork(), recurse=True)
